...
### 已记录、暂缓：Architecture V3（2026-07-21，parked）

同一 source 内多表 + many-to-one JOIN，走向可开源产品。评估、路线（视图 v0 →
声明式 JOIN）、星型约束与打包缺口见 [`docs/architecture-v3.md`](docs/architecture-v3.md)。
Hackathon（07-23）前不动 compiler；JOIN 工作等待有已知答案的真实多表数据集。
